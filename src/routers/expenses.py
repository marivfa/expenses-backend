...
async def create_expenses(expenses: schemas_expenses.Expenses, db: Session = Depends(get_db), id_user: int = Depends(get_current_user)):
    try:
        db_expenses = crud_expenses.create_expenses(
            db=db, expenses=expenses, id_user=id_user)
        if expenses.remainders is not None:
            crud_remainders.create_remainders(
                db=db, remainders=expenses.remainders)
        return db_expenses
...
async def get_expenses(skip: int = 0, limit: int = 25, db: Session = Depends(get_db), id_user: int = Depends(get_current_user)):
    expenses = crud_expenses.get_expenses(
...
async def get_data_excel(db: Session = Depends(get_db), id_user: str = Depends(get_current_user)):
    expenses = crud_expenses.get_expenses(
        db, 0, 0, id_user=id_user, download='xls')
    df = pd.DataFrame(
        expenses,
        columns=["User", "Category", "Amount",
                 "Comment", "Date register", "Real date"]
    )
    ...
    try:
        s3_resource.upload_file(file_name, os.environ.get(
            'S3_NAME'), f"public/{file_name}")
    except Exception as e:
        raise HTTPException(
            status_code=404, detail="Error uploading file: "+str(e))
...
async def delete_expenses(expenses_id: int, db: Session = Depends(get_db)):
    ...
    if db_expenses is None:
        raise HTTPException(status_code=404, detail="Expenses Not found")
    if crud_expenses.delete_expenses(db=db, expenses_id=expenses_id):
...
async def update_expenses(expenses_id: int, expenses: schemas_expenses.Expenses, db: Session = Depends(get_db)):
    try:
        db_expenses = crud_expenses.update_expenses(
            db, expenses_id=expenses_id, expenses=expenses)
        return db_expenses
    except IntegrityError as e:
        raise HTTPException(status_code=400, detail="Invalid input data")
    except Exception as e:
